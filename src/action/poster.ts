...
const uploadImage = async (file: File): Promise<string> => {
    try {
        ...
        console.log("🖼️ Upload success, URL:", result.url);
        return result.url;
    } catch (err: any) {
...
const deleteImage = async (imageUrl: string): Promise<void> => {
    try {
        ...
        if (!result.success) {
            console.error("🖼️ Error deleting image from Cloudflare:", result.message);
        }
    } catch (err) {
    ...
    try {
        ...
        if (imageFile && imageFile.size > 0) {
            if (existingPoster.image_url) {
                await deleteImage(existingPoster.image_url);
            }
...
export const deletePoster = async (id: string) => {
    try {
        ...
        if (poster.image_url) {
            await deleteImage(poster.image_url);
        }
