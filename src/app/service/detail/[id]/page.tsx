...
import { type ICategory, type IPackageCategories, type IImageCategories, type IComment, IIncludes } from '@/interface';
...
export default function CategoryDetailPage({ params }: CategoryDetailPageProps) {
  ...
  const [feature, setFeature] = useState<IIncludes[]>([]);
  // Cart states
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const featureResult = await getIncludesByCategory(categoryId);
    ...
    <div className="min-h-screen py-6 md:py-8 mt-16 md:mt-20">
      ...
      <div className="max-w-7xl mx-auto bg-white px-4 sm:px-5">
        ...
        <div className="grid md:grid-cols-2 gap-6 md:gap-10 mb-8">
          {/* Image Gallery */}
          <div className="space-y-3 md:space-y-4 max-w-xs sm:max--w-sm md:max-w-sm  xl:max-w-xl mx-auto">
            {images.length > 0 ? (
              <>
                      ...
                      {images.map((image, index) => (
                        ...
                        >
                          <button
                            ...
                            className={` w-16 h-16 sm:w-20 sm:h-20 rounded-lg overflow-hidden border-2 transition-all ${currentImageIndex === index
                              ? 'border-purple-500 '
          ...
          <div className="w-full">
            <div className="flex flex-col h-full">
              <div className="space-y-4 md:space-y-6 flex-1">
                <div>
                  <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold  text-eliane  mb-3 md:mb-4 mt-1 md:mt-5">
                    {category.name}
                  </h1>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {feature.map((feature, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <span className="text-gray-700 text-md text-arial sm:text-base">{feature.include_name}</span>
                  </div>
                ))}
              </div>
            </CardContent>
        ...
        <div className="mt-12 md:mt-20 p-2 sm:p-5">
          <h3 className="text-xl sm:text-2xl md:text-4xl font-bold mb-4 text-borsok text-[#50398e] relative inline-block">
            How To Order
                  ...
                  <CardHeader>
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                      ...
                      <Badge className="bg-purple-100 text-purple-700 w-fit">
                        {comment.order_items?.package_title || 'Verified Purchase'}
                      </Badge>
                    </div>
                  </CardHeader>
                  <CardContent>
